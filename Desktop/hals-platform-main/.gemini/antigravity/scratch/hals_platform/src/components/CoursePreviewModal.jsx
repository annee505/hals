...
const CoursePreviewModal = ({ course, onClose, onEnroll }) => {
    ...
    React.useEffect(() => {
        ...
        if (content) {
            // Parse actual course duration (e.g. "8 weeks" -> 8)
            const totalWeeks = parseInt(course.duration) || 4;

            // Collect all lessons from all modules
            const allLessons = content.modules.flatMap(mod =>
                mod.lessons.map(l => ({ title: l.title, duration: l.duration }))
            );

            // Distribute lessons evenly across the actual number of weeks
            const weeks = [];
            const lessonsPerWeek = Math.max(1, Math.ceil(allLessons.length / totalWeeks));

            for (let w = 0; w < totalWeeks; w++) {
                const start = w * lessonsPerWeek;
                const weekLessons = allLessons.slice(start, start + lessonsPerWeek);

                // If we've run out of lessons, create a review/practice week
                const topics = weekLessons.length > 0
                    ? weekLessons.map(l => l.title)
                    : [`Review & Practice - Week ${w + 1}`];

                // Show realistic weekly study hours (lectures + practice + assignments)
                const topicCount = topics.length;
                const baseHours = topicCount <= 1 ? 2 : topicCount <= 2 ? 3 : 4;
                const weekDuration = weekLessons.length > 0
                    ? `${baseHours}-${baseHours + 2} hrs`
                    : '2-3 hrs';

                // Generate a descriptive title for each week
                const weekTitles = [
                    'Introduction & Setup', 'Core Concepts', 'Building Skills',
                    'Hands-On Practice', 'Deep Dive', 'Real-World Application',
                    'Advanced Techniques', 'Project Work', 'Optimization',
                    'Integration', 'Review & Polish', 'Final Project'
                ];

                weeks.push({
                    week: w + 1,
                    title: weekTitles[w % weekTitles.length],
                    topics,
